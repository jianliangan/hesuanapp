...
router.beforeEach(async (to, from, next) => {
	...
	if (!isGetRouter) {
		...
		let userMenu = treeFilter(userRoutes, (node: keyable) => {
...
//入侵追加自定义方法、对象
(router as unknown as GetMenu_in).sc_getMenu = () => {
...
function filterAsyncRouter(routerMap: any) {
	const accessedRouters = new Array<any>()
...
//路由扁平化
function flatAsyncRoutes(routes: any, breadcrumb = new Array<any>()) {
...
//过滤树
function treeFilter(tree: any[], func: (arg0: keyable) => any) {

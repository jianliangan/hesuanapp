		...
		<el-aside width="200px" v-loading="showasideing">
			<el-container>
				...
				<el-main class="nopadding">
					<el-tree :data="organizedata" :props="groupsProps" @node-click="leftRowClick"></el-tree>
				</el-main>
			</el-container>
		</el-aside>
		<el-container>
			...
			<el-main class="nopadding">
				...
				<el-table v-loading="loading" :data="tableData" row-key="id" border default-expand-all stripe
					style="width: 100%">
				...
				<div style="float:right;margin: 20px;">
					<el-pagination layout="prev, pager, next" :total="flowListNum" :page-size="flowPageSize" small
...
import { OrganizeFetchList, OrganizePushRow, UserFetchList } from '@/api/model/setting'
class ListUriParams {
	page: number = 1
	id: string = ""
}
...
const Get_Data_list = async (row: any) => {
	loading.value = true
	dialogAddVisible.value = true
	OrganizeFetchList(row).then((response: any) => {
...
const Get_Data_list_middle = async (row: any) => {
	...
	UserFetchList(row).then((response: any) => {
		tableData.value.splice(0)
